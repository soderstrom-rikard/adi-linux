...
static irqreturn_t bf53xPFbuttons_irq_handler(int irq, void *dev_id)
{
	...
	bf53xPFbuttons->laststate[i] ^= bf53xPFbuttons->statechanged[i];
		...
		input_report_key(bf53xPFbuttons->dev, bf53xPFbuttons->btncode[i],bf53xPFbuttons->laststate[i]?0:1);
		...
		case EV_LED:
			...
			for (i=0; i<LEDS; ++i) {
				if (bf53xPFbuttons->ledcode[i] == code) {
					gpio_set_value(bf53xPFbuttons->led_gpios[i], value);
		...
		case EV_SND:
			...
			for (i=0; i<BELLS; ++i) {
				if (bf53xPFbuttons->sndcode[i] == code) {
					gpio_set_value(bf53xPFbuttons->snd_gpios[i], value);
					bf53xPFbuttons->events_processed++;
					return 0;
				}
			}
...
static void inline bf53xPFbuttons_remove_IRQ(struct bf53xPFbuttons *bf53xPFbuttons)
{
	...
	for(i=0; i<BUTTONS; i++)
	{
    		free_irq(IRQ_PF0 + bf53xPFbuttons->btn_gpios[i], bf53xPFbuttons_irq_handler);
	}
...
static int inline bf53xPFbuttons_init_IRQ(struct bf53xPFbuttons *bf53xPFbuttons)
{
	...
	for(i=0; i<BUTTONS; i++)
	{
			...
			if (request_irq(IRQ_PF0 + bf53xPFbuttons->btn_gpios[i], bf53xPFbuttons_irq_handler, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "bf53xPFbuttons", bf53xPFbuttons)) {
			    /* Rollback */
...
static int bf53xPFbuttons_request_gpio(unsigned short *gpios,unsigned short nr_gpios)
{
	u16 i;

	for(i=0; i<nr_gpios; i++)
		if (gpio_request(gpios[i], NULL)) {
			printk(KERN_WARNING "bf53xPFbuttons: Failed to request GPIO_%d\n",gpios[i]);
		} else {
...
static int bf53xPFbuttons_release_gpio(unsigned short *gpios,unsigned short nr_gpios)
{
	u16 i;
...
static int bf53xPFbuttons_proc_output(struct bf53xPFbuttons *bf53xPFbuttons, char *buf)
{
	...
	for (i = 0; i < MAX_BLACKFIN_GPIOS; i++)
		p += sprintf(p, "PF%d\t: %d....%d....%d....%d....%d....%d.....%d.....%d \n", i, get_gpio_data(i), get_gpio_dir(i),get_gpio_inen(i),get_gpio_edge(i),get_gpio_both(i), get_gpio_polar(i), get_gpio_maska(i), get_gpio_maskb(i));
