...
static int adv7511_s_register(struct v4l2_subdev *sd, struct v4l2_dbg_register *reg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int adv7511_g_chip_ident(struct v4l2_subdev *sd, struct v4l2_dbg_chip_ident *chip)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static long adv7511_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_SUBDEV_G_EDID: {
		...
		memcpy(edid->edid, &state->edid.data[edid->start_block * 128], 128*edid->blocks);
		return 0;
	}
...
static int test(struct v4l2_subdev *sd)
{
	adv7511_wr(sd, 0x1, 0x0);
	adv7511_wr(sd, 0x2, 0x18);
	adv7511_wr(sd, 0x3, 0x0);
	adv7511_wr(sd, 0x15, 0x01);
	adv7511_wr(sd, 0x16, 0x0fc);
	adv7511_wr(sd, 0x18, 0x46);
	adv7511_wr(sd, 0x40, 0x80);
	adv7511_wr(sd, 0x41, 0x10);
	adv7511_wr(sd, 0x48, 0x10);
	adv7511_wr(sd, 0x49, 0xa8);
	adv7511_wr(sd, 0x4c, 0x0);
	adv7511_wr(sd, 0x55, 0x20);
	adv7511_wr(sd, 0x56, 0x08);
	adv7511_wr(sd, 0x96, 0x20);
	adv7511_wr(sd, 0x98, 0x3);
	adv7511_wr(sd, 0x99, 0x2);
	adv7511_wr(sd, 0x9c, 0x30);
	adv7511_wr(sd, 0x9d, 0x61);
	adv7511_wr(sd, 0x35, 0x40);
	adv7511_wr(sd, 0x36, 0xd9);
	adv7511_wr(sd, 0x37, 0x0a);
	adv7511_wr(sd, 0x38, 0x0);
	adv7511_wr(sd, 0x39, 0x2d);
	adv7511_wr(sd, 0x3a, 0x0);
	adv7511_wr(sd, 0xd7, 0x1b);
	adv7511_wr(sd, 0xd8, 0x82);
	adv7511_wr(sd, 0xd9, 0x80);
	adv7511_wr(sd, 0xda, 0x14);
	adv7511_wr(sd, 0xdb, 0x05);
	adv7511_wr(sd, 0x17, 0x01);
	adv7511_wr(sd, 0xa2, 0xa4);
	adv7511_wr(sd, 0xa3, 0xa4);
	adv7511_wr(sd, 0xa5, 0x44);
	adv7511_wr(sd, 0xab, 0x40);
	adv7511_wr(sd, 0xaf, 0x16);
	adv7511_wr(sd, 0xba, 0x60);
	adv7511_wr(sd, 0xd1, 0xff);
	adv7511_wr(sd, 0xde, 0x9c);
	adv7511_wr(sd, 0xe4, 0x60);
	adv7511_wr(sd, 0xfa, 0x7d);
}
/* ------------------------------ VIDEO OPS ------------------------------ */
...
static int adv7511_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#if 0
	state->hdmi_mode_ctrl = v4l2_ctrl_new_custom(hdl, &adv7511_ctrl_hdmi_mode, NULL);
	state->audio_sample_freq_ctrl = v4l2_ctrl_new_custom(hdl, &adv7511_ctrl_audio_sample_freq, NULL);
	state->audio_word_length_ctrl = v4l2_ctrl_new_custom(hdl, &adv7511_ctrl_audio_word_length, NULL);
	state->audio_channel_count_ctrl = v4l2_ctrl_new_custom(hdl, &adv7511_ctrl_audio_channel_count, NULL);
	state->audio_channel_map_ctrl = v4l2_ctrl_new_custom(hdl, &adv7511_ctrl_audio_channel_map, NULL);
	state->audio_i2s_format_ctrl = v4l2_ctrl_new_custom(hdl, &adv7511_ctrl_audio_i2s_format, NULL);
	state->hotplug_ctrl = v4l2_ctrl_new_custom(hdl, &adv7511_ctrl_hotplug, NULL);
	state->rx_sense_ctrl = v4l2_ctrl_new_custom(hdl, &adv7511_ctrl_rx_sense, NULL);
	state->have_edid0_ctrl = v4l2_ctrl_new_custom(hdl, &adv7511_ctrl_edid_segment0, NULL);
	state->rgb_quantization_range_ctrl = v4l2_ctrl_new_custom(hdl, &adv7511_ctrl_rgb_quantization_range, NULL);
#endif
	sd->ctrl_handler = hdl;
	if (hdl->error) {
		err = hdl->error;
	...
	state->edid_i2c_client = i2c_new_dummy(client->adapter, (state->edid_addr>>1));
